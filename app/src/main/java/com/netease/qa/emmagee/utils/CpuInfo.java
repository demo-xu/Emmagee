	...
	public ArrayList<String> getCpuRatioInfo(String totalBatt, String currentBatt, String temperature, String voltage, String fps, boolean isRoot) {
		...
		try {
			...
			if (isInitialStatics) {
				...
				int[] pidUss = mi.getPidUssSize(pid, context);
				...
				if (isPositive(processCpuRatio) && isPositive(totalCpuRatio.get(0))) {
                    ...
                    EmmageeService.bw
                            .write(mDateTime2 + Constants.COMMA + ProcessInfo.getTopActivity(context) + Constants.COMMA
                                    + heapData + pMemory
                                    + Constants.COMMA + percent + Constants.COMMA + fMemory + Constants.COMMA
                                    + processCpuRatio + Constants.COMMA
                                    + totalCpuBuffer.toString() + trafValue + Constants.COMMA + totalBatt
                                    + Constants.COMMA + currentBatt + Constants.COMMA
                                    + temperature + Constants.COMMA + voltage + Constants.COMMA + fps + Constants.COMMA
                                    + pidUss[0] + Constants.COMMA + pidUss[1] + Constants.COMMA + pidUss[2]
                                    + Constants.LINE_END);
