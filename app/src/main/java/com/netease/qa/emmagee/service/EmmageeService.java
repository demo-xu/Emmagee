...
public class EmmageeService extends Service {
    ...
    private static final int RES_ID = R.layout.mail_settings;
	@Override
	public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        try {
            ...
            notification = builder.setSmallIcon(R.drawable.icon).setTicker("正在导航")
		...
		if (isFloating) {
            ...
            mUssTextView = viFloatingWindow.findViewById(R.id.mem_uss);
            ...
            mUssTextView.setTextColor(android.graphics.Color.RED);
	...
	private void createResultCsv() {
		...
		try {
			...
			bw.write(getString(R.string.timestamp) + Constants.COMMA
                    ...
                    + Constants.COMMA + "dalvikUss"
                    + Constants.COMMA + "nativeUss"
                    + Constants.COMMA + "otherUss"
					+ Constants.LINE_END);
		} catch (IOException e) {
...
//        wmParams.format = 1;
	...
	private void dataRefresh() {
	    int[] pidUss = memoryInfo.getPidUssSize(pid, getBaseContext());
		...
		if (isFloating) {
			...
			if (!processInfo.isEmpty()) {
				...
				mUssTextView.setText("dalvikUss " + pidUss[0] + "nativeUss " + pidUss[1]);
