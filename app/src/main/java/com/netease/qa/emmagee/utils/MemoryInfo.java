...
public class MemoryInfo {
    ...
    public int[] getPidUssSize(int pid, Context context) {
        ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        int[] myMempid = new int[] {pid};
        Debug.MemoryInfo[] memoryInfo = am.getProcessMemoryInfo(myMempid);
        int dalvikUss = memoryInfo[0].dalvikPrivateDirty;
        int nativeUss = memoryInfo[0].nativePrivateDirty;
        int other = memoryInfo[0].otherPrivateDirty;
        return new int[] {dalvikUss, nativeUss, other};
    }
